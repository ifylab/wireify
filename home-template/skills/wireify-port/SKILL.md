...
1. **Read the original.** For a wired chain (a whole definition, or a region of native components feeding the script), open with ONE `get_document_graph` call — every node with its type identity and every wire as an edge, `includeOutputs: true` for the live values — instead of one introspect per component. Then on the component itself: `introspect_selected`; **`get_source`** for its IronPython 2 code; `read_input_data` on its live inputs (read-only calls — batch them in one message) so you know the real types, tree shape, and samples (not what the code assumes). Persistent values of UNWIRED native inputs are never exposed: read the component's OUTPUT (a Rotate's angle, a Mirror's plane show in the transform output's text).
